...
void loop(){ 
  ...
  if(myserial.available() > 0) {    //if we see that the pH Circuit has sent a character.
    //Serial.println("character reveived from ph-circuit");
...
//    Serial.println("nothing receiced jet");
//    lcd.print("NaN"); 
  }
  lcd.setCursor(6,1);
...
void Arduino_Control(){
  ...
  //delay(3000);                         //we will take a reading ever 3000ms. You can make this much longer or shorter if you like.
  ...
  if(string_received==1){            //did we get data back from the ph Circuit?
    ph=atof(ph_data); 
    Serial.println('PH: ');
    Serial.println(ph_data);     //many people ask us "how do I convert a sting into a float?" This is how...
    getTemperature();
